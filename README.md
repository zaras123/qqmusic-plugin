...
| 🎵 点歌 | 公共账号回落 | 群友自己没登录时，点歌回落到配置的「公共账号」（通常是主人登录的 VIP 号）——全群都能播 VIP 曲；留空=不启用 |
...
> 📌 **关于「公共账号」**：默认每个群友点歌都用自己的登录态（`userKey` = 各自 QQ 号），没登录就是免登录状态、只能播免费曲。
> 在锅巴或 `config/config/qqmusic.yaml` 里把 `publicAccount` 填成**某个已登录账号的 QQ 号**（通常是主人扫码登录的绿钻号），
> 之后**没登录的群友**点歌会自动回落到这个账号 —— 即全群都能播 VIP 曲。填了但那个号没登录时会静默失效，`#qqm设置` 会标红提示。
>
> 只影响「播歌 / 取数据」；登录状态、取 CK、刷新这些仍只认请求者本人（否则会把别人的登录态误报成本人的）。访客 Token 也无法指定公共账号。
...
- 公共账号（留空=不启用；填已登录账号的 QQ 号 → 没登录的群友点歌回落到它，全群都能播 VIP 曲）
- 从 API 一键回填登录态
