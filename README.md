...
| 🎵 点歌 | 主人账号回落 | 群友自己没登录时，点歌回落到配置的「主人账号」（通常是主人登录的 VIP 号）——全群都能播 VIP 曲；留空=不启用。「一律走主人账号」默认开：所有人点歌都按主人的 ck，锅巴可关 |
...
> 在锅巴或 `config/config/qqmusic.yaml` 里把 `publicAccount`（主人账号）填成**主人已登录账号的 QQ 号**（通常是主人扫码登录的绿钻号），
> 之后**没登录的群友**点歌会自动回落到这个账号 —— 即全群都能播 VIP 曲。填了但那个号没登录时会静默失效，`#qqm设置` 会标红提示。
>
> 只影响「播歌 / 取数据」；登录状态、取 CK、刷新这些仍只认请求者本人（否则会把别人的登录态误报成本人的）。访客 Token 也无法指定主人账号。
>
> **想让所有人点歌都按主人的 ck 走？**「一律走主人账号」（`forceMasterAccount`）**默认已开启**：
> 所有人点歌都按主人账号取播放链，不看请求者自己登录没有（群友在共用同一 API 的自己机器人上
> 登录过非 VIP 号也不影响）。登录状态、收藏、刷新仍按请求者本人。
> 生效前提是先填 `publicAccount`，没填时开关等于没开（日志会提示一次）；不需要全员共用主人号时在锅巴关闭即可。
...
- 主人账号（配置键 `publicAccount`，留空=不启用；填主人已登录账号的 QQ 号 → 没登录的群友点歌回落到它，全群都能播 VIP 曲）
- 一律走主人账号（默认开；所有人点歌都按主人账号的 ck，不看请求者自己登录没有，锅巴可关）
- 从 API 一键回填登录态
